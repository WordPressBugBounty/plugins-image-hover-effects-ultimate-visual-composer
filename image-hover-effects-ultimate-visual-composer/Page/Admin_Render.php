...
use OXI_FLIP_BOX_PLUGINS\Classes\Controls;
    ...
    public function child_save() {
        if ( ! empty( $_POST['oxi-flip-template-modal-submit'] ) && $_POST['oxi-flip-template-modal-submit'] == 'Submit' ) {
            if ( ! wp_verify_nonce( $this->nonce, 'oxiflipchildnonce' ) ) {
                ...
                if ( $id == '' ) {
                    $this->wpdb->query( $this->wpdb->prepare( "INSERT INTO {$this->child_table} ( files, styleid) VALUES ( %s, %d)", [ $child, $this->oxiid ] ) );
                } elseif ( $id != '' && is_numeric( $id ) ) {
                    $item_id = (int) $id;
                    $this->wpdb->update( "$this->child_table", [ 'files' => $child ], [ 'id' => $item_id ], [ '%s' ], [ '%d' ] );
                }
            }
        }
    }
